...
function CmsSidebar({ activeTab, open, onClose, onSelect }: { activeTab: Tab; open: boolean; onClose: () => void; onSelect: (tab: Tab) => void }) {
  const select = (tab: Tab) => { onSelect(tab); onClose(); };
  const settings = tabs.find((item) => item.id === "settings")!;
  return <><button type="button" aria-label="Close navigation" tabIndex={open ? 0 : -1} onClick={onClose} className={`fixed inset-0 z-40 bg-navy-950/55 backdrop-blur-sm transition-opacity lg:hidden ${open ? "opacity-100" : "pointer-events-none opacity-0"}`} /><aside className={`fixed inset-y-0 left-0 z-50 flex w-[18rem] flex-col bg-navy-950 text-cream-50 shadow-2xl transition-transform duration-300 lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}><div className="flex h-20 items-center justify-between border-b border-white/10 px-5"><Link to="/" target="_blank" rel="noopener noreferrer" className="group"><p className="label-caps text-leaf-300">AFhomes</p><p className="mt-1 text-sm font-semibold text-cream-50">Content Studio</p></Link><button type="button" onClick={onClose} className="rounded-lg p-2 text-cream-200 hover:bg-white/10 lg:hidden" aria-label="Close sidebar">×</button></div><nav className="no-scrollbar flex-1 overflow-y-auto px-3 py-5" aria-label="CMS navigation">{(["Manage website", "Library & activity", "More content"] as const).map((group) => <div key={group} className="mb-6"><p className="label-caps mb-2 px-3 text-cream-300/45">{group}</p><div className="space-y-1">{tabs.filter((item) => item.group === group && item.id !== "settings").map((item) => <SidebarButton key={item.id} item={item} active={activeTab === item.id} onSelect={select} />)}</div></div>)}</nav><div className="border-t border-white/10 p-3"><SidebarButton item={settings} active={activeTab === "settings"} onSelect={select} /><p className="mt-3 px-3 text-[0.66rem] text-cream-300/40">Role-ready workspace · Admin access</p></div></aside></>;
}
