...
type Tab = "dashboard" | "pages" | "hero" | "rooms" | "amenities" | "gallery" | "videos" | "offers" | "testimonials" | "faq" | "contact" | "seo" | "media" | "history" | "inbox" | "stories" | "vip" | "settings";
interface TabItem { id: Tab; label: string; icon: string; group: "Manage website" | "Library & activity" | "More content" }
const tabs: TabItem[] = [
  { id: "dashboard", label: "Dashboard", icon: "dashboard", group: "Manage website" },
  { id: "pages", label: "Pages / Website Content", icon: "pages", group: "Manage website" },
  { id: "hero", label: "Hero Section", icon: "hero", group: "Manage website" },
  { id: "rooms", label: "Rooms / Accommodations", icon: "rooms", group: "Manage website" },
  { id: "amenities", label: "Amenities", icon: "amenities", group: "Manage website" },
  { id: "gallery", label: "Gallery / Photos", icon: "gallery", group: "Manage website" },
  { id: "videos", label: "Videos", icon: "videos", group: "Manage website" },
  { id: "offers", label: "Promotions / Offers", icon: "offers", group: "Manage website" },
  { id: "testimonials", label: "Testimonials / Reviews", icon: "testimonials", group: "Manage website" },
  { id: "faq", label: "FAQs", icon: "faq", group: "Manage website" },
  { id: "contact", label: "Contact & Locations", icon: "contact", group: "Manage website" },
  { id: "seo", label: "SEO", icon: "seo", group: "Manage website" },
  { id: "media", label: "Media Library", icon: "media", group: "Library & activity" },
  { id: "history", label: "Change History / Restore", icon: "history", group: "Library & activity" },
  { id: "inbox", label: "Inquiry Inbox", icon: "inbox", group: "Library & activity" },
  { id: "stories", label: "Stories", icon: "stories", group: "More content" },
  { id: "vip", label: "VIP Privilege", icon: "vip", group: "More content" },
  { id: "settings", label: "Settings", icon: "settings", group: "Manage website" },
];
const tabAliases: Record<string, Tab> = { overview: "dashboard", site: "settings", experiences: "rooms", backups: "history" };

function SidebarButton({ item, active, onSelect }: { item: TabItem; active: boolean; onSelect: (tab: Tab) => void }) {
  return <button type="button" title={item.label} aria-current={active ? "page" : undefined} onClick={() => onSelect(item.id)} className={`group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[0.82rem] font-semibold transition-colors ${active ? "bg-leaf-500 text-pine-950 shadow-subtle" : "text-cream-200/75 hover:bg-white/8 hover:text-cream-50"}`}><AdminIcon name={item.icon} className="h-[1.15rem] w-[1.15rem] shrink-0" /><span>{item.label}</span>{active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-pine-900" aria-hidden="true" />}</button>;
}

function CmsSidebar({ activeTab, open: requestedOpen, onClose: dismiss, onSelect }: { activeTab: Tab; open: boolean; onClose: () => void; onSelect: (tab: Tab) => void }) {
  const [open, setOpen] = useState(requestedOpen);
  useEffect(() => { if (requestedOpen) setOpen(true); }, [requestedOpen]);
  const onClose = () => { setOpen(false); dismiss(); };
  const select = (tab: Tab) => { onSelect(tab); onClose(); };
  const settings = tabs.find((item) => item.id === "settings")!;
  return <><button type="button" aria-label="Open navigation" onClick={() => setOpen(true)} className={`fixed top-5 left-4 z-30 grid h-10 w-10 place-items-center rounded-xl border border-line bg-cream-50 text-pine-900 shadow-soft transition-opacity hover:bg-leaf-100 ${open ? "pointer-events-none opacity-0" : "opacity-100"}`}><span className="block h-0.5 w-5 bg-current" /><span className="block h-0.5 w-5 bg-current" /><span className="block h-0.5 w-5 bg-current" /></button><button type="button" aria-label="Close navigation" tabIndex={open ? 0 : -1} onClick={onClose} className={`fixed inset-0 z-40 bg-navy-950/55 backdrop-blur-sm transition-opacity ${open ? "opacity-100" : "pointer-events-none opacity-0"}`} /><aside className={`fixed inset-y-0 left-0 z-50 flex w-[18rem] flex-col bg-navy-950 text-cream-50 shadow-2xl transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"}`}><div className="flex h-20 items-center justify-between border-b border-white/10 px-5"><Link to="/" target="_blank" rel="noopener noreferrer" className="group"><p className="label-caps text-leaf-300">AFhomes</p><p className="mt-1 text-sm font-semibold text-cream-50">Content Studio</p></Link><button type="button" onClick={onClose} className="rounded-lg p-2 text-cream-200 hover:bg-white/10" aria-label="Close sidebar">×</button></div><nav className="no-scrollbar flex-1 overflow-y-auto px-3 py-5" aria-label="CMS navigation">{(["Manage website", "Library & activity", "More content"] as const).map((group) => <div key={group} className="mb-6"><p className="label-caps mb-2 px-3 text-cream-300/45">{group}</p><div className="space-y-1">{tabs.filter((item) => item.group === group && item.id !== "settings").map((item) => <SidebarButton key={item.id} item={item} active={activeTab === item.id} onSelect={select} />)}</div></div>)}</nav><div className="border-t border-white/10 p-3"><SidebarButton item={settings} active={activeTab === "settings"} onSelect={select} /><p className="mt-3 px-3 text-[0.66rem] text-cream-300/40">Role-ready workspace · Admin access</p></div></aside></>;
}
...
const internalRoutes = ["/", "/about", "/experiences", "/experiences/hotspring-ecofarm-resort", "/experiences/smart-wellness-hotel", "/experiences/alm-japanese-restaurant", "/vip", "/stories", "/faq", "/contact"];
...
function Field({ label, value, onChange, area = false, type = "text" }: { label: string; value: string | number; onChange: (value: string) => void; area?: boolean; type?: string }) {
  ...
  if (label === "Path") return <label className="block"><span className="label-caps text-ink-500">Destination</span><select value={value} onChange={(event) => onChange(event.target.value)} className="cms-input">{[...new Set([String(value), ...internalRoutes])].map((route) => <option key={route} value={route}>{route === "/" ? "Home" : route}</option>)}</select></label>;
  if (type === "url") return <div className="block"><span className="label-caps text-ink-500">{label}</span><p className="mt-2 text-sm text-ink-600">Managed from the media library.</p></div>;
...
function ImageField({ label, image, onChange }: { label: string; image: ImageSpec; onChange: (image: ImageSpec) => void }) {
  ...
  return <div className="rounded-xl border border-line bg-cream-100/50 p-4 sm:col-span-2"><div className="flex flex-col gap-4 sm:flex-row sm:items-end"><div className="min-w-0 flex-1"><Field label={label} value={image.src} onChange={(value) => onChange({ ...image, src: value })} type="url" /><p className="mt-2 text-xs text-ink-400">Use a compressed WebP or JPEG, ideally under 1 MB. The centered crop is previewed below.</p></div><div className="flex gap-2"><label className="inline-flex cursor-pointer items-center justify-center rounded-full border border-line px-5 py-3 text-sm font-semibold text-pine-900 hover:border-pine-800 hover:bg-pine-800 hover:text-cream-50"><span>{uploading ? "Uploading..." : image.src ? "Replace" : "Upload image"}</span><input disabled={uploading} type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void readFile(file); event.target.value = ""; }} /></label>{image.src && <button type="button" className="px-3 text-sm font-semibold text-coral-700" onClick={() => onChange({ ...image, src: "" })}>Remove</button>}</div></div><div className="mt-4 grid gap-4 sm:grid-cols-[8rem_1fr] sm:items-center"><div className="aspect-4/3 overflow-hidden rounded-lg border border-line bg-cream-200">{image.src ? <img src={image.src} alt="" className="h-full w-full object-cover" /> : <span className="flex h-full items-center justify-center text-xs text-ink-400">No image</span>}</div><Field label="Image description / alt text" value={image.alt} onChange={(value) => onChange({ ...image, alt: value })} /></div></div>;
}
...
function Section({ title, description, children, onSave }: { title: string; description: string; children: ReactNode; onSave: () => Promise<void> }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-line bg-cream-50 shadow-soft"><fieldset disabled={saving} className="p-5 sm:p-8"><div className="flex flex-col justify-between gap-5 border-b border-line pb-6 lg:flex-row lg:items-end"><div><p className="label-caps text-leaf-700">Content editor</p><h2 className="font-display mt-1 text-4xl font-medium text-navy-900">{title}</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-ink-600">{description}</p><p className="mt-2 text-xs font-semibold text-pine-800">Changes publish when you select Update.</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" size="sm" onClick={() => window.dispatchEvent(new Event("afhomes-cms-updated"))}>Cancel</Button><Button type="button" variant="outline" size="sm" onClick={() => window.open("/", "_blank", "noopener,noreferrer")}>Preview</Button><Button type="button" variant="accent" size="sm" onClick={() => void save()}>{saving ? "Updating..." : "Update"}</Button></div></div>{error && <p role="alert" className="mt-4 rounded-lg bg-coral-100 px-4 py-3 text-sm font-semibold text-coral-700">{error}</p>}{children}</fieldset></section>;
}
export default function Admin() {
  ...
  const tab = tabAliases[requestedTab] ?? tabs.find((item) => item.id === requestedTab)?.id ?? "dashboard";
  ...
  const notify = (message: string) => { setNotice(""); const error = /failed|error|unable|invalid|required|must|could not/i.test(message); void showToast(message, error ? "error" : "success"); };
  const activeLabel = tabs.find((item) => item.id === tab)?.label ?? "Dashboard";
  return <DraftContext.Provider value={markDirty}><div className="min-h-screen bg-cream-100 text-ink-800"><CmsSidebar activeTab={tab} open={sidebarOpen} onClose={() => setSidebarOpen(false)} onSelect={setTab} /><div className="min-h-screen lg:pl-[18rem]"><header className="sticky top-0 z-30 border-b border-line bg-cream-50/95 backdrop-blur"><div className="flex h-20 items-center justify-between gap-4 px-4 sm:px-7 lg:px-10"><div className="flex min-w-0 items-center gap-3"><button type="button" className="rounded-xl border border-line p-2.5 text-pine-900 lg:hidden" onClick={() => setSidebarOpen(true)} aria-label="Open CMS navigation"><span className="block h-0.5 w-5 bg-current" /><span className="mt-1.5 block h-0.5 w-5 bg-current" /><span className="mt-1.5 block h-0.5 w-5 bg-current" /></button><div className="min-w-0"><p className="label-caps text-ink-400">AFhomes CMS</p><h1 className="truncate font-display text-2xl font-medium text-navy-900">{activeLabel}</h1></div></div><div className="flex items-center gap-3"><span role="status" className={`hidden rounded-full px-3 py-1.5 text-xs font-semibold sm:inline-flex ${dirty ? "bg-gold-100 text-ink-700" : "bg-leaf-100 text-pine-900"}`}><span className={`mr-2 mt-1 h-1.5 w-1.5 rounded-full ${dirty ? "bg-gold-600" : "bg-leaf-500"}`} />{dirty ? "Unsaved changes" : "Up to date"}</span><Link to="/" target="_blank" rel="noopener noreferrer" className="hidden text-sm font-semibold text-pine-800 hover:text-leaf-700 md:block">View site ↗</Link><button type="button" className="text-sm font-semibold text-ink-500 hover:text-coral-700" onClick={async () => { if (dirty && !window.confirm("Discard your unsaved changes and sign out?")) return; try { await logoutAdmin(); window.location.assign("/admin/login"); } catch { notify("Sign out failed. Please try again."); } }}>Sign out</button></div></div></header>{notice && <div className="fixed right-4 top-24 z-40 max-w-sm rounded-xl border border-leaf-300 bg-pine-900 px-5 py-4 text-sm font-semibold text-cream-50 shadow-2xl" role="status" aria-live="polite">{notice}</div>}<main className="mx-auto w-full max-w-[100rem] px-4 py-6 sm:px-7 sm:py-8 lg:px-10 lg:py-10">{tab === "dashboard" && <Overview onSelect={setTab} />}{tab === "inbox" && <InquiryInbox />}{tab === "media" && <MediaLibrary />}{tab === "history" && <BackupPanel />}{tab === "settings" && <SettingsEditor notify={notify} />}{tab === "contact" && <SettingsEditor notify={notify} initialPanel="locations" />}{tab === "hero" && <HeroEditor notify={notify} />}{tab === "seo" && <SeoEditor notify={notify} />}{tab === "pages" && <VisualPageBuilder />}{tab === "amenities" && <VisualPageBuilder focusBlock="features" heading="Amenities" description="Add and arrange amenity or service sections on any managed page." />}{tab === "gallery" && <VisualPageBuilder focusBlock="gallery" heading="Gallery / Photos" description="Build photo galleries, manage captions and alt text, then preview before publishing." />}{tab === "videos" && <VisualPageBuilder focusBlock="video" heading="Videos" description="Add visitor-facing videos to any page and control playback settings." />}{tab === "offers" && <VisualPageBuilder focusBlock="cta" heading="Promotions / Offers" description="Create and publish promotional calls to action without changing page URLs." />}{tab === "testimonials" && <VisualPageBuilder focusBlock="testimonials" heading="Testimonials / Reviews" description="Add, arrange, preview, and publish guest testimonials." />}{tab === "rooms" && <ExperiencesEditor notify={notify} />}{tab === "vip" && <VipEditor notify={notify} />}{tab === "faq" && <FaqEditor notify={notify} />}{tab === "stories" && <StoriesEditor notify={notify} />}</main></div></div></DraftContext.Provider>;
}
...
function Overview({ onSelect }: { onSelect: (tab: Tab) => void }) {
  ...
  const missingAlt = experiences.filter((item) => item.image.src && !item.image.alt.trim()).length + stories.filter((item) => item.cover.src && !item.cover.alt.trim()).length;
  const missingSeo = 11 - new Set(config.pageSeo.filter((item) => item.metaDescription && item.openGraphImage.src).map((item) => item.path)).size;
  const cards: [string, string, Tab, string][] = [["Edit homepage hero", "Update the primary message and hero media", "hero", "hero"], ["Manage website pages", "Use reusable blocks, drafts, preview, and publish", "pages", "pages"], ["Upload media", "Add optimized photos and videos to the library", "media", "media"], ["Review SEO", `${Math.max(0, missingSeo)} pages need attention`, "seo", "seo"]];
  return <div><section className="rounded-2xl bg-pine-900 p-7 text-cream-50 shadow-lift sm:p-10"><div className="flex flex-col justify-between gap-7 lg:flex-row lg:items-end"><div><p className="label-caps text-leaf-300">Good day, AFhomes team</p><h2 className="font-display mt-3 text-4xl font-medium sm:text-5xl">Your website at a glance.</h2><p className="mt-3 max-w-2xl text-cream-200/70">Manage content, keep visitor details accurate, and publish confidently from one calm workspace.</p></div><Button variant="accent" onClick={() => onSelect("pages")}>Edit website content</Button></div></section><div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl border border-line bg-cream-50 p-5"><p className="label-caps text-ink-400">Experiences</p><p className="font-display mt-2 text-4xl text-navy-900">{experiences.filter((item) => !item.archived).length}</p><p className="mt-1 text-sm text-ink-500">Active visitor experiences</p></div><div className="rounded-2xl border border-line bg-cream-50 p-5"><p className="label-caps text-ink-400">FAQs</p><p className="font-display mt-2 text-4xl text-navy-900">{cmsRepository.getAllFaqCategories().reduce((total, category) => total + category.items.length, 0)}</p><p className="mt-1 text-sm text-ink-500">Published answers</p></div><div className="rounded-2xl border border-line bg-cream-50 p-5"><p className="label-caps text-ink-400">Last content update</p><p className="font-display mt-2 text-2xl text-navy-900">{recent?.[0] ? new Date(recent[0].saved_at).toLocaleDateString() : "No edits yet"}</p><p className="mt-1 text-sm text-ink-500">{recent?.[0]?.change_summary ?? "History begins after the first save"}</p></div><button type="button" onClick={() => onSelect(missingAlt ? "media" : "seo")} className={`rounded-2xl border p-5 text-left ${missingAlt || missingSeo > 0 ? "border-gold-300 bg-gold-100/60" : "border-leaf-300 bg-leaf-100"}`}><p className="label-caps text-ink-500">Content health</p><p className="font-display mt-2 text-2xl text-navy-900">{missingAlt + Math.max(0, missingSeo)} items</p><p className="mt-1 text-sm text-ink-600">Missing SEO details or image alt text</p></button></div><section className="mt-8"><div className="flex items-end justify-between"><div><p className="label-caps text-leaf-700">Quick actions</p><h3 className="font-display mt-1 text-3xl text-navy-900">What would you like to update?</h3></div></div><div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map(([title, detail, id, icon]) => <button key={id} type="button" onClick={() => onSelect(id)} className="group rounded-2xl border border-line bg-cream-50 p-5 text-left shadow-subtle transition hover:-translate-y-0.5 hover:border-leaf-500 hover:shadow-soft"><span className="inline-flex rounded-xl bg-leaf-100 p-2.5 text-pine-800"><AdminIcon name={icon} className="h-5 w-5" /></span><p className="font-display mt-4 text-2xl font-medium text-navy-900">{title}</p><p className="mt-2 text-sm leading-relaxed text-ink-500">{detail}</p><span className="mt-5 inline-block text-sm font-semibold text-pine-800">Open editor →</span></button>)}</div></section>{recent && recent.length > 0 && <section className="mt-8 rounded-2xl border border-line bg-cream-50 p-6"><div className="flex items-center justify-between gap-4"><h3 className="font-display text-2xl text-navy-900">Recent edits</h3><button type="button" className="text-sm font-semibold text-pine-800" onClick={() => onSelect("history")}>View history</button></div><ul className="mt-4 divide-y divide-line">{recent.slice(0, 5).map((row) => <li key={row.id} className="flex flex-col justify-between gap-1 py-3 sm:flex-row"><span className="text-sm text-ink-700">{row.change_summary}</span><time className="text-xs text-ink-400">{new Date(row.saved_at).toLocaleString()}</time></li>)}</ul></section>}</div>;
}
