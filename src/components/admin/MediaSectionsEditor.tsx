...
import { cmsMediaPages, type CmsMediaBlock, type CmsMediaKind, type CmsMediaPlacement, type CmsMediaWidth } from "@/types/mediaBlock";

const emptyBlock = (): CmsMediaBlock => ({
  id: crypto.randomUUID(), page: "/", placement: "after-page", kind: "image", src: "", alt: "", caption: "", width: "wide", fit: "cover",
});
...
export function MediaSectionsEditor({ notify }: { notify: (message: string) => void }) {
  ...
  const save = async () => {
    ...
    try { await cmsRepository.saveMediaBlocks(blocks); notify("Page media saved"); }
  ...
  return <section className="border border-line bg-cream-50 p-6 sm:p-9">
    <div className="flex flex-col justify-between gap-4 border-b border-line pb-6 sm:flex-row sm:items-end"><div><p className="label-caps text-leaf-700">Page builder</p><h2 className="font-display mt-1 text-3xl font-medium text-navy-900">Images & videos</h2><p className="mt-2 text-sm text-ink-600">Add ordered media sections before or after any page. Upload a file or paste a public URL.</p></div><Button disabled={saving} variant="accent" size="sm" onClick={() => void save()}>{saving ? "Saving…" : "Save changes"}</Button></div>
