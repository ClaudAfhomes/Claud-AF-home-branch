...
export function MediaSectionsEditor({ notify }: { notify: (message: string) => void }) {
  ...
  const upload = async (block: CmsMediaBlock, file: File) => {
    ...
    } catch (cause) { await showError(cause instanceof Error ? cause.message : "Upload failed"); }
