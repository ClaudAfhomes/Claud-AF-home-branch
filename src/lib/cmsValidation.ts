...
const mediaBlocks = z.array(z.object({
  ...
  placement: z.enum(["before-page", "after-page"]),
...
export const documentSchemas = {
  site: z.object({ siteUrl: z.string().url().regex(/^https?:\/\//), brand: z.object({ name: text, tagline: text, mantra: text }), email: z.string().email(), phone: text, phoneDisplay: text, offices: z.array(z.object({ name: text, role: text, lines: z.array(text) })), nav: z.object({ main: z.array(nav), experiences: z.array(nav) }) }),
  ...
  pageContent: z.object({
    ...
    experiences: z.object({ ...heading, image }), vip: z.object({ ...heading, tierEyebrow: text, tierTitle: text, tierLede: text, steps: z.array(z.object({ number: text, title: text, body: text })), includedEyebrow: text, includedTitle: text, includedLede: text, includedButton: text, transparencyEyebrow: text, transparencyTitle: text, transparencyLede: text, transparencyButton: text }), stories: z.object({ ...heading, image }),
  }),
};
