...
export interface LocalRevision { id: string; key: DocumentKey; value: unknown; revision: number; saved_at: string }
...
export async function listHistory(page: number) { return readStore().history.slice(page * 25, page * 25 + 25); }
...
export async function saveDocuments(changes: Partial<Record<DocumentKey, unknown>>) {
  const save = () => {
    const store = readStore();
    for (const key of Object.keys(changes) as DocumentKey[]) {
      ...
      if (store.documents[key] !== undefined) store.history.unshift({ id: crypto.randomUUID(), key, value: store.documents[key], revision: store.versions[key] ?? 0, saved_at: new Date().toISOString() });
